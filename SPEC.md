...
- 启动: navigate /sessions/1/settle (登录 demo@example.com / code 000000)
...
### v0.3.27 — UAT 0723-2 Batch 2 续 (PO msg 16:35 #8469 续 #11 + #18)

2 项 handdrawn-arrow EmptyState 引导:

- [x] **#11 /sessions EmptyState 手绘箭头 → 新建账本 FAB** — push `3f5d61e` (sessions/+page.svelte). PO msg 16:35 字面: 「我的账本页面, 没有任何账本时, 目前会有文字提示。加一个手绘风格的箭头, 由左上至右下, 指向"新建账本"按钮(以起到引导的作用)」. 修法: EmptyState 内嵌 SVG path (120×220 viewBox, 2 段 bezier 曲线 + 三角箭头). fixed 位置 bottom:175px left:24px. z-index:49 (.fab z=50 之下不挡点击). pointer-events:none (不影响 EmptyState / FAB 交互). stroke-dasharray='5,4' + opacity 0.78 + var(--accent-500) 色让箭头手绘轻质感不抢视觉. rotate 8deg 增加手绘感.
- [x] **#18 /sessions/[id] bills EmptyState 手绘箭头 → 新建账单 FAB** — push `e9efc66` (sessions/[id]/+page.svelte). PO msg 16:35 字面: 「账单列表页没有账单时, 加一个手绘风格的箭头, 由左上至右下, 指向"新建账单"按钮(以起到引导的作用)」. 修法: bills EmptyState 内嵌 SVG path (跟 #11 同款 120×220 viewBox, 2 段 bezier 曲线 + 三角箭头). fixed 位置 bottom:175px left:24px (跟 #11 全站手绘箭头视觉一致). /sessions/[id] 页面有 nav bar + sticky header, fixed 让箭头与 FAB 视觉对齐不随滚动乱跑. z-index:49 + pointer-events:none 同 #11. stroke-dasharray + opacity + var(--accent-500) 同款.

Playwright iPhone 13 验证 (`frontend/scripts/v0327-arrow-verify.cjs`):
- ✓ PART A 新 user 0 sessions 状态: EmptyState visible + .handdrawn-arrow present + position fixed + z-index 49 + pointer-events none + width ~110 (rotate 8° 包围盒 137) + height ~213 (rotate bbox) + x=11 left + centerY=568 bottom region + FAB x=290 right of arrow + FAB y=744 below arrow center
- ✓ PART B xinhua1001 0-bills session 6 状态: bills EmptyState visible + .handdrawn-arrow-bills present + 同款 position/z-index/pointer-events/w/h/x + centerY + FAB 位置关系
- ✓ 截图 2 张存 `~/.openclaw/media/browser/v0327-arrow/` (image tool 视觉确认 EmptyState + 蓝紫色虚线 wavy 箭头 + 三角箭头头 清晰可见)

svelte-check: 2 errors / 20 warnings (baseline 同, 0 new error — pre-existing 在 +page.svelte:553 `session_code` 和 join/+page.svelte:32 `SessionPreviewMember`, 跟 #11/#18 无关)
单分支铁律: origin 仅 main (committed `3f5d61e` + `e9efc66`, push bf7fe5c..e9efc66)

### v0.3.27 — UAT 0723-2 Batch 2 续 排除范围 (本任务不修, 待 PO 决定)
- 箭头终点离 FAB 中心 ~200px 距离 + rotate 8°, 视觉方向对(左上→右下) 但**未精准命中 FAB center** — image tool 实判 #11 箭头"明确指向右下角 FAB" 但 #18 箭头"指向描述文字而非 FAB". 后续可 spawn Designer 调位置/path/rotate 精准命中, 等 PO 反馈
- arrow 仅在 EmptyState 0 sessions / 0 bills 状态出现 — 已有 sessions / bills 时不渲染 (EmptyState 才渲染, 跟着渲染)
