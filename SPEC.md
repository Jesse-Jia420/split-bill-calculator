...
- [x] **#4 我的账本按钮位置** — push 待定 (lib/components/NavBar.svelte). PO 字面意图 "header 里的「我的账本」按钮, 放在 用户名和「注销登录」按钮的中间. 注意留出合理的间隔". 修法: 删 .links 块 + 「我的账本」<a> 从 .links 移到 .right 里, 位置在 用户名 <span> 和 注销登录 <button> 之间. CSS 删 .links { flex:1; ... } + .links a { display:inline-flex; ... } 整块 (dead code). .right 默认 gap: var(--space-2) (4px) — 但「我的账本」跟 用户名 / 注销登录 之间是 8px (navbar gap var(--space-3) — 实际测). 保留 wizard 时不显示我的账本 (page.url.pathname !== '/sessions/new'). 排除范围: .right 内三元素的 gap 沿用原 var(--space-2), 不额外加大 (PO 说 "合理间隔" — 4-8px 已合理). 全局 navbar 视觉不变 (SplitIt logo 位置不动).
