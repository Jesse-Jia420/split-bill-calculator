...
// NOTE: T04 relies on the dev seed (scripts/seed_dev_data.py) which creates
// session 1 with xinhua1001@outlook.com as the owner. We intentionally do NOT
// call wipeDb() here — full_flow.spec.ts already wipes+seeds its own session,
// and T04 needs session 1 to already exist with xinhua as a member.
...
test("T04 fix: swipe-delete bill → click 撤销 → bill restored", async ({
  ...
  // 2) Create a uniquely-named bill via the API directly (faster than
  //    navigating to the new-bill page).
  const uniqueDesc = `t04-fix-${Date.now()}`;
  const created = await page.evaluate(async (desc) => {
    const res = await fetch("/api/sessions/1/bills", {
      method: "POST",
      credentials: "include",
      ...
      body: JSON.stringify({
        ...
        currency: "THB",
        participants: [{ member_id: 1, is_exclusive: false, exclusive_amount: 0 }],
      }),
    });
  ...
  await page.goto("http://localhost:8448/sessions/1", {
  ...
  const deleteRespPromise = page.waitForResponse(
    (r) =>
      r.url().endsWith(`/api/sessions/1/bills/${billId}`) &&
  ...
  const postRespPromise = page.waitForResponse(
    (r) =>
      r.url().endsWith("/api/sessions/1/bills") &&
  ...
  const restoredCount = await page.evaluate(async (desc) => {
    const res = await fetch("/api/sessions/1/bills", {
      credentials: "include",
    });
