...
(async () => {
  ...
  console.log('[v0728-3-3] Step 1: login as demo@example.com');
  ...
  const checks = [
    ...
    { name: 'hint display = flex (v3 block-level for margin-left:auto)', pass: hintStyles?.display === 'flex' },
    { name: 'hint align-self = auto (v3 删, 用 margin-left:auto 替代)', pass: hintStyles?.alignSelf === 'auto' },
