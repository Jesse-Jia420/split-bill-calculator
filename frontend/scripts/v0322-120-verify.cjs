...
async function main() {
  ...
  await page.evaluate(async () => {
    await fetch('https://test.jessejia.pp.ua/api/auth/send-code', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({email:'demo@example.com'})});
    await fetch('https://test.jessejia.pp.ua/api/auth/verify-code', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({email:'demo@example.com',code:'000000'}), credentials: 'include'});
  });
