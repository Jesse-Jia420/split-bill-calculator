...
(async () => {
  ...
  // Login via API
  const r1 = await page.request.post('http://127.0.0.1:8449/auth/send-code', { data: { email: 'demo@example.com' } });
  console.log('send-code:', r1.status());
  const r2 = await page.request.post('http://127.0.0.1:8449/auth/verify-code', { data: { email: 'demo@example.com', code: '000000' } });
