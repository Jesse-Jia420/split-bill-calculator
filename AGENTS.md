...
### Dev auth + seed data
- Login is passwordless: request a code, then submit any 6-digit code. Emails listed in `DEV_BYPASS_EMAILS` (currently `demo@example.com`, `demo@example.com`) skip SMTP entirely and accept any code — use these for browser/QA flows (no real inbox needed).
- With `SBC_SKIP_SEED=false`, startup idempotently seeds `demo@example.com` + a Thailand test session (40 bills, THB+CNY) + an empty personal session (`backend/scripts/seed_dev_data.py`).
