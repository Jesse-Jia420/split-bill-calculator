...
- **Away reload skip:** `shouldSkipAwayReload` in `$lib/utils/skipAwayReload` — `/auth/login` and `/sessions/{id}/login` skip the global visibility hard-reload (OTP step). Bill-list `/s/{code}` still hard-reloads.
